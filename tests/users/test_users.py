...
class TestUsers:
    ...
    def test_create_user(self, domain: str, public_users_client: PublicUsersClient):
        ...
        response = public_users_client.create_user_api(request)  # Получаем ответ от апи после создания клиента
    ...
    def test_get_user_me(self, private_users_client: PrivateUsersClient, function_user: UserFixture):
        ...
        assert_status_code(response.status_code, HTTPStatus.OK)  # проверка статус кода ответа
        assert_get_user_response(response_data, function_user.response)  # проверка тела ответа
        validate_json_schema(response.json(), response_data.model_json_schema())  # валидация json схемы